...
注意力队列已按「含 blocked 优先，其余按等待最久优先」排序。呼叫卡与清算面板已共享 mockup 测量出的材质、圆角、颜色和项目人格；下一步补齐终态条目、受限项目组与静默运行态。继续用真实 hook 回合观察项目别名覆盖率与 workflow marker 使用率；若需要让未映射项目的完成可见，先由用户显式加入别名表。只有在用户明确反馈需要时，才重新调整「完成轻响 / 卡住语音」的双声音默认。
