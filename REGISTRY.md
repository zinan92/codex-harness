...
注意力队列已按「含 blocked 优先，其余按等待最久优先」排序。呼叫卡与清算面板已共享 mockup 测量出的材质、圆角、颜色和项目人格；终态条目显示等待时间和本轮账，同项目默认最多显示三条并可展开。清算面板还会安静显示已映射项目的运行时长，既不计数也不主动弹出。菜单栏 App 现由用户 LaunchAgent 在登录与异常退出后恢复；一次真实 Claude 非零退出验证了 `SessionEnd` 会把未结束的 Work Unit 收束为 blocked，不会假装持续运行。
