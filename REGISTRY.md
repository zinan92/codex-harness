...
- 每个 Claude/Codex 读数现在带可见的数据口径：本地来源、今日范围、共同刷新时间与 fresh/stale/unavailable 状态；任一来源不可用时，合计明确标为不完整而非零值。
...
- 首页增加了与合计、Codex、Claude 口径同步的「今天下一步」决策卡：明确下一段行动、理由和节奏；未读取的数据只会提示等待可信扫描。
- 展开面板可把近期 Claude/Codex 会话关联到项目、工作类型和实际产出；会话标记只保存在 `~/.tokenpulse/session-annotations.json`，不上传、不进入 Git。
- 展开面板现在显示今天的本地会话时间线：按真实更新时间排序、标出超过 45 分钟的间隔和 Claude/Codex 工具切换；它只使用会话元数据，不推断实际产出。
- 展开面板增加本地 7 天复盘：展示活跃日、用户已记录产出和一个下一步；token 只被视为活动量，绝不被伪称为成果。
