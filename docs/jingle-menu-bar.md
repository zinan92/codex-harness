...
Jingle 的展示单位是 Codex session，而不是单个 Work Unit。若某个未归档 session 的 running Work Unit 之后尚未出现 transcript `task_complete` 终态事件，它只会安静地显示一次「工作中」：同 session 所有旧 `done` / `blocked` 记录都会退出清算队列、自动呼叫和菜单数字。`task_complete` 之后的旧回合也不会复活成待办；只有尚未被 session 级完成事件覆盖的终态才可能进入「等你」。因此不会把一个长任务同时显示成“仍在跑”和“等你决定”；SQLite 的 UI 更新时间不用于结束长任务判断。同一 thread 的多条遗留 running 记录只显示一次。若 cwd 是共享目录且没有显式项目 alias，Jingle 只使用 Codex session-index 的短名称作临时标签，绝不读取或持久化 SQLite 中常为原始 prompt 的 `title`。`ignored_prefixes` 可显式排除 Jingle 自身等不应监控的 cwd。
